//===--- RavynOSSDKInfo.cpp - SDK Information parser for darwin - ----------===//
//
...
Optional<VersionTuple> RavynOSSDKInfo::RelatedTargetVersionMapping::map(
...
Optional<RavynOSSDKInfo::RelatedTargetVersionMapping>
RavynOSSDKInfo::RelatedTargetVersionMapping::parseJSON(
...
Optional<RavynOSSDKInfo>
RavynOSSDKInfo::parseRavynOSSDKSettingsJSON(const llvm::json::Object *Obj) {
  ...
  return RavynOSSDKInfo(std::move(*Version),
                       std::move(*MaximumDeploymentVersion),
                       std::move(VersionMappings));
}

Expected<Optional<RavynOSSDKInfo>>
clang::parseRavynOSSDKInfo(llvm::vfs::FileSystem &VFS, StringRef SDKRootPath) {
  ...
  if (const auto *Obj = Result->getAsObject()) {
    if (auto SDKInfo = RavynOSSDKInfo::parseRavynOSSDKSettingsJSON(Obj))
